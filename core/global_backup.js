...
function showMenuBar()
{
    ...
    _this.add.sprite(87, 494, inGame_MenuLeft);
    _this.add.sprite(0, 66, inGame_BarTimer).setOrigin(0, 0.5).setTint('0xa3a3a3');
   ...
   // _this.add.sprite(192, 66, inGame_Player);
    ...
    if (currAvatarIsBoy === 'true') {
      var headIcon =  _this.add.sprite(204.5, 65, 'head');
      headIcon.setScale(0.3);
      headIcon.depth = 10;
      headIcon.flipX = true;
      headIcon.mask = new Phaser.Display.Masks.BitmapMask(_this, mask);
    } else {
        var headIcon =  _this.add.sprite(203.5, 80, 'head');
        headIcon.setScale(0.3);
        headIcon.depth = 10;
        headIcon.flipX = true;
        headIcon.mask = new Phaser.Display.Masks.BitmapMask(_this, mask);
...
function loadHelp()
{
    _this.load.image(currentScene + '_help', getSceneImagesFolder() + currentScene +  '_help.png');
    loadButtonsExtra();
}

function createHelp()
{
    helpObject = _this.add.image(0, 0, currentScene + '_help').setOrigin(0, 0); 
...
var lvl1, lvl2, lvl3;
var introPanelBubble, introPanelTextImage, introBackground;
...
function loadLevelSelectPanel()
{
    ...
    _this.load.image('footer_menu', imagesGeneral + 'footer_menu.png');
...
function showLevelSelectPanel()
{
    ...
    function onEvent() {
        ...
        if (intoTextsRead == true) {
            //timer.destroy();
            console.info(" text are loaded!");
            LoadIntroTexts();
            introTextTimer.destroy();
            introTextsRead = false;
        } else {
            console.info(" text are not loaded!");
        }
    }

    intoTextsRead = false;
    var introTextTimer;
    introTextTimer = _this.time.addEvent({ delay: 100, callback: onEvent, callbackScope: this, loop: true });
    ...
    footer_menu = _this.add.image(959, 1022, 'footer_menu');
